...
class ProxyResolver(BaseResolver):
    ...
    def __init__(self,address,port,timeout,ip_range,cleanup_interval,cleanup_expiry):
        ...
        rule = "iptables -w -t nat -nL RANDN_VPN-MAIN-MAPPING | awk '{if (NR<3) {next}; print $5, substr($6, 4)}'"
        ...
        for mapping in mappings.split("\n"):
            if mapping:
                ...
                if not self.mapping_ip(real_ip,fake_ip,current_time):
                    rule = "iptables -w -t nat -F RANDN_VPN-MAIN-MAPPING"
    ...
    def get_fake_ip(self,real_ip):
        with self.lock:
            ...
            if entry:
                ...
                rule = f"iptables -w -t nat -A RANDN_VPN-MAIN-MAPPING -d {fake_ip} -j DNAT --to {real_ip}"
                subprocess.run(rule,shell=True,check=True)
    ...
    def cleanup_fake_ips(self):
        with self.lock:
            ...
            for real_ip,fake_ip in cleanup_ips:
                ...
                rule = f"iptables -w -t nat -D RANDN_VPN-MAIN-MAPPING -d {fake_ip} -j DNAT --to {real_ip}"
                subprocess.run(rule,shell=True,check=True)
